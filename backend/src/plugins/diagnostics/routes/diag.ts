...
const diagRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  // GET /pipeline — last sync per source
  ...
  // GET /volume — load/assignment/dispatch counts
  fastify.get("/volume", async (_request, reply) => {
    const db = fastify.db;
    if (!db)
      return reply.status(503).send({
        success: false,
